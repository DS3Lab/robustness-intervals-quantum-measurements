...
from lib.helpers import make_paulicliques, estimate_ground_state_fidelity
...
class EigenvalueInterval(RobustnessInterval):
    ...
    def compute_interval(self, variables, backend, device, noise, samples, fidelity=None, alpha=1e-2, nreps=20):
        if fidelity is None:
            ...
            # compute fidelity
            fidelity = estimate_ground_state_fidelity(eigvals, eigvecs, self._ansatz, variables, backend, device, noise, samples)
